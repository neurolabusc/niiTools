...
function DoxPowell(X, Y    : DoubleP;
                   Lb, Ub ,vox : Integer;
                   MaxIter : Integer;
                   Tol     : Double; var B: DoubleP): double;
function GammaFit_InitialGuess(X, Y    : DoubleP; Lb, Ub   : Integer; var B : DoubleP): boolean;

function xGammaFit_RSqr(X, Y    : DoubleP; Lb, Ub  : Integer; B : DoubleP) : Float;
...
function DoxPowell(X, Y    : DoubleP;
                   Lb, Ub,vox  : Integer;
                   MaxIter : Integer;
                   Tol     : Float; var B: DoubleP): double;
...
begin
  ...
  result := maxint;
  if  (P[1]<0) or (P[1]>P[2])or ( (P[2] - P[1]) <= 0) or (P[2]>=MaxX) or (P[4]>MaxInt)or (P[4]<=0) then
    exit;
...
1:ITER:=ITER+1;
  ...
  IF ITER = ITMAX Then
  begin
    showmessage(' Powell exceeding maximum iterations: '+inttostr(vox));
    goto 10
  end;
  For J:=1 to N do
  begin
