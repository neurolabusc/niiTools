...
uses
 ...
 nii_core, Classes, SysUtils, define_types, otsuml;
const
     kMorphDeleteLargestDark=0; //extract multiple objects
     kMorphPreserveLargestBright= 1; //extract single object
     kMorphPreserveLargestBrightAndInclusions = 2; //extract single object, do not mask dark bubbles inside object
...
procedure PreserveLargestCluster (var lImg: Bytep; lXi,lYi,lZi, lMinClusterSize: integer; lClusterValue,ValueForSmallClusters: byte  );
procedure BubbleFill (var lImg: Bytep; lXi,lYi,lZi: integer; lAirValue,lFillVal: byte  );
procedure MaskBackground  (var lImg: Bytep; lXi,lYi,lZi,lMinClusterSize, lOtsuLevels,lMode: integer; lDilateVox: single );

implementation

(*procedure Noise (var lImg: Bytep; lXi,lYi,lZi: integer);
var
   n,x,y,z: integer;
begin
     n := 0;
...
begin
   ...
   riteln('Otsu level '+inttostr(lOtsuLevels)+' Min cluster size ' + inttostr(lMinClusterSize)+' Mode '+inttostr(lMode));
   SmoothFWHM2Vox(lMask, lXi,lYi,lZi);
   if (lOtsuLevels = 0 ) then begin
     //riteln('Otsu''s method bypassed');
     for lV := 1 to lXYZ do
          if lMask^[lV] <= 0 then
             lMask^[lV] := 0
          else
   ...
   if lMode <> kMorphDeleteLargestDark then begin
      PreserveLargestCluster (lMask, lXi,lYi,lZi,lMinClusterSize,255,0 ); //only preserve largest single object
     if lDilateVox >= 1 then
         DilateSphere (lMask, lXi,lYi,lZi,lDilateVox,255 );
     if lMode = kMorphPreserveLargestBrightAndInclusions then
        BubbleFill (lMask, lXi,lYi,lZi,0 ,255);
   end else begin
         if lDilateVox >= 1 then
            DilateSphere (lMask, lXi,lYi,lZi,lDilateVox,255 );
         PreserveLargestCluster (lMask, lXi,lYi,lZi,lMinClusterSize,0,255 ); //only erase outside air
   end;
   //Noise (lMask, lXi,lYi,lZi);
   lV:=0;
   for lZ := 1 to lZi do
       for lY := 1 to lYi do
           for lX := 1 to lXi do begin
             inc(lV);
             //if (lX=1) or (lX=lXi) or (lY=1) or (lY=lYi) or (lZ=1) or (lZ=lZi) then
                //
             //else
                   if (lMask^[lV] = 0)  then
                  lImg^[lV] := 0;
...
begin
  ...
  if (lXi < 5) or (lYi < 5) or (lZi < 1) then exit;
   ...
   for lX := 1 to lXYZ do
     ...
     riteln(inttostr(lC)+ ' of '+inttostr(lXYZ)+' voxels before clustering');
  if lC < 2 then
      exit;//e.g. if lC = 1 then only a single voxel, which is in fact largest cluster

  riteln(inttostr(lXi)+' '+inttostr(lYi)+'  '+inttostr(lZi)   );
  getmem(lTemp,lXYZ*sizeof(longint));
   ...
   if (lMinClusterSize > 0) and (lC > lMinClusterSize) then
      lC := lMinClusterSize;
...
procedure EdgeFill (var lImg: Bytep; lXi,lYi,lZi: integer; lAirValue,lFillVal: byte);
//Given volume lImg, set all voxels with airvalue that are connected to an edge with fillval
var
  lStart: DWord;
  lTopSlice,lInc,lXY,lXYZ,lClusterSign,lQTail,lQHead,lQSz,lClusterSz,lClusterFillValue: integer;
    lQra: LongIntP;
...
procedure Check(lPixel: integer);
 begin
    if (lImg^[lPixel]=lAirValue) then begin//add item
        incQra(lQHead,lQSz);
        lQra^[lQHead] := lPixel;
        lImg^[lPixel] := lFillVal;
...
//var lI: integer;
begin
  if (lImg^[lPt]<>lAirValue) then exit;
  lQHead := 0;
  lQTail := 1;
  Check(lPt);
  RetirePixel;
...
begin //proc CountClusterSize
  ...
  for lZ := 1 to lZi do
      for lY := 1 to lYi do
          for lX := 1 to lXi do begin
              inc(lV);
              if (lX=1) or (lX=lXi) or (lY=1) or (lY=lYi) or (lZ=1) or (lZ=lZi) then //edge
                 FillStart(lV);
          end;
...
begin
   ...
   //ensure at least some voxels exist with clusterValue
   lX := 1;
   while (lX <= lXYZ) and (lImg^[lX] = lAirValue) do
         inc (lX);
   if lX > lXYZ then
      exit;//no air, hence no bubble
   //ensure at least some voxels exist without clusterValue
   lX := 1;
   while (lX <= lXYZ) and (lImg^[lX] <> lAirValue) do
         inc (lX);
   if lX > lXYZ then
      exit;//no non-air, hence no bubble
   getmem(lTemp,lXYZ*sizeof(byte));
   Move(lImg^[1], lTemp^[1],lXYZ);
   EdgeFill(lTemp,lXi,lYi,lZi,lAirValue,lFillVal);
   //at this stage only bubbles in lTemp still have airvalue
   lC := 0;
   for lX := 1 to lXYZ do
       if (lImg^[lX] = lAirValue) and (lTemp^[lX] = lAirValue) then begin
          lImg^[lX] := lFillVal; //fill the bubbles
          inc(lC);
       end;
   riteln(inttostr(lC)+ ' voxels are bubbles and will not be masked.');
   freemem(lTemp);
end;
