...
function PermutedZ (lnObservations,lnFactors,lFactorToTest,lnPermute: integer;var Xf: PMatrix; var Y: PVector; lPermOrder: PIntMatrix; var lMaxZ,lMinZ: PVector; lUnmaskedObs: longIntp): double;
label
  666;
var
  lP,lF,lO,lDF,lRan,lnOK : integer;
  X    : PMatrix;
...
begin
  ...
  for lP := 1 to lnPermute do begin
      lnOK := 0;
   	  lO := 0;
   	  while lnOK < lnObservations do begin
      		inc(lO);
       		lRan := lUnmaskedObs^[lPermOrder^[lP]^[lO]];
            if lRan > 0 then begin //valid
               inc(lnOK);
               for lF := 1 to (lnFactors) do begin
          	   	   if lRan > lnObservations then
           	  	   	  fx(6666666);
        	  	   X^[lF]^[lnOK] := Xf^[lF]^[lRan ];
      	  	   end; //for each factor

            end; //this participant is not masked
    end;
...
function MultipleRegressionPermutedZ (lnObservations,lnFactors,lFactorToTest,lnPermute: integer;  var Xf: PMatrix; var Y: PVector; lPermOrder: PIntMatrix; var lMaxZ,lMinZ: PVector; lUnmaskedObs: longIntp): double;
label
  666;
var
...
begin
  ...
  if lnFactors < 2 then begin
     result := PermutedZ (lnObservations,lnFactors,lFactorToTest,lnPermute, Xf, Y,lPermOrder, lMaxZ,lMinZ,lUnmaskedObs);
      exit;
  end;
  ...
  if not ReturnResiduals (lnObservations,lnFactors-1, X, Y, Yresidual, lOutSlope) then
    goto 666;
  //permute here...
  result := PermutedZ (lnObservations,lnFactors,lFactorToTest,lnPermute, Xf, Yresidual,lPermOrder, lMaxZ,lMinZ,lUnmaskedObs);
...
begin
  ...
  if not GetValReg (lVALFilename, lnS,lnFactors,X, lImages, lPredictorList) then
     ...
     if (lnThread > 0) and (lnPermute > 1) then begin
     ...
     Getmem(lOutT,(lnFactors+1)*sizeof(double));
     Getmem(lOutSlope,(lnFactors+1)*sizeof(double));
     Getmem(lUnmaskedObs,lnS*sizeof(longInt));
              ...
              if (lnThread > 0) and (lnPermute > 1) then begin
                if (lV mod 50) = 0 then begin
         		 ...
         		 application.processmessages;
               	...
               	lNIIx.f32^[lV] := MultipleRegressionPermutedZ (lnObs,lnFactors,1,lnPermute, Xdata, Y, lPermOrder, lMaxZ^[1],lMinZ^[1],lUnmaskedObs)
              end else begin
     ...
     freemem(lUnmaskedObs);
     freemem(lOutT);
     freemem(lOutSlope);
     DelMatrix(Xdata, lnFactors, lnS);
     if (lnThread > 0) and (lnPermute > 1) then
